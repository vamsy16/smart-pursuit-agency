...
**Built (2026-09-03):** `public-guard.sh` (CI + local: refuses client data, a `client:` name in the public zone, a public
ticket merged without a gate score, secrets, PII, licences, oversized files in a public repo) and `vault.sh` (seal / open / cat / rotate the encrypted ledger — tested both ways).

**To build, in this order:** `scaffold-client.mjs` (create the 15 folders inside the vault + MANIFEST) →
